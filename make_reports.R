...
plan = drake_plan(
  ...
  results_1 = get_results_from_folder(1),
  data_1 = preprocess_results(results_1$oob.perf %>% filter(search.type %in% c("design", "package-default", "random"))),
  ...
  results_2 = get_results_from_folder(2),
  data_2 = preprocess_results(results_2$oob.perf %>% filter(search.type %in% c("design", "package-default", "random"))),
  plots_2 = make_plots(results_2, data_2, 2),
  report_2 = create_report(learner.name_2, data_2, 2),
  # svm
  learner.name_4 = paste0(learner.names[4]),
  results_4 = get_results_from_folder(4),
  data_4 = preprocess_results(results_4$oob.perf %>% filter(search.type %in% c("design", "package-default", "random"))),
  plots_4 = make_plots(results_4, data_4, 4),
  report_4 = create_report(learner.name_4, data_4, 4),
  # xgboost
  learner.name_6 = paste0(learner.names[6]),
  results_6 = get_results_from_folder(6),
  data_6 = preprocess_results(results_6$oob.perf %>% filter(search.type %in% c("design", "package-default", "random"))),
